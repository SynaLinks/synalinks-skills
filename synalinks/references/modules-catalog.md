...
Retrieve entities from knowledge base by similarity search.

```python
outputs = await synalinks.EntityRetriever(
...
Retrieve relationship triplets from knowledge graph.

```python
outputs = await synalinks.TripletRetriever(
    knowledge_base=kb,
    language_model=lm,
    return_inputs=True,
)(inputs)
...
Add/update entities in knowledge base.
